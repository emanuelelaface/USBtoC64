/*
CUSTOMIZATION FOR Retro Bit Bluetooth Controller, USB 121:290
...
void c64_joystick_j(const uint8_t *const data, const int length) {
  ...
  if ((data[6] == 31) | (data[6] == 47) | (data[7] == 8)) {
      isfire = true;
  }
  if (isup) {
    pinMode(C64_UP, OUTPUT);
