/*
CUSTOMIZATION FOR Xbox One For Windows, USB 1118:767
...
void c64_joystick_j(const uint8_t *const data, const int length) {
  ...
  if ((data[10] == 255) | (data[10] == 0) | (data[11] == 2)) {
      isfire = true;
  }
  if (data[11] == 4) {
    autofire = true;
...
void c64_joystick_m(const uint8_t *const data, const int length) {
  ...
  if ((data[1] < 64) | (data[1] > 190)){
    delayOnX += (data[1]-127)/3;
  }
  if ((data[5] < 64) | (data[5] > 190)){
    delayOnX += (data[5]-127)/3;
  }
  ...
  if ((data[7] < 64) | (data[7] > 190)){
    delayOnY += (127-data[7])/3;
  }
